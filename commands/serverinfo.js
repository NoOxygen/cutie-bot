exports.run = async (client, message) => {
	...
	let embed = new Discord.MessageEmbed()
            ...
            .addField("Roles Count", `${message.guild.roles.cache.size}`)
						.addField("Text Channels", `${message.guild.channels.cache.filter((c) => c.type === "text").size}`)
						.addField("Voice Channels", `${message.guild.channels.cache.filter((c) => c.type === "voice").size}`)
						.addField("Created On", `${moment.utc(message.guild.createdAt).format("MMMM Do YYYY")}`)
